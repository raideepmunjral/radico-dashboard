      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span>8PM Sales Mix</span>
                <span>{data.summary.totalSales > 0 ? ((data.summary.total8PM / data.summary.totalSales) * 100).toFixed(1) : '0.0'}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-purple-600 h-2 rounded-full transition-all duration-500" 
                  style={{ width: `${data.summary.totalSales > 0 ? (data.summary.total8PM / data.summary.totalSales) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span>VERVE Sales Mix</span>
                <span>{data.summary.totalSales > 0 ? ((data.summary.totalVERVE / data.summary.totalSales) * 100).toFixed(1) : '0.0'}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  className="bg-orange-600 h-2 rounded-full transition-all duration-500" 
                  style={{ width: `${data.summary.totalSales > 0 ? (data.summary.totalVERVE / data.summary.totalSales) * 100 : 0}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t">
              <h4 className="font-medium text-gray-700 mb-2">Market Penetration</h4>
